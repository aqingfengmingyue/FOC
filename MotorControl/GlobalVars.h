...
typedef struct
{
    ...
    float z_alpha;     // 滑模控制信号α轴
    float z_beta;      // 滑模控制信号β轴
...
typedef struct
{
    ...
    float omega_est; // 估算速度
    float err_prev;  // 上一次误差(用于滤波)
} PLL_HandleTypeDef;
...
extern LPF_HandleTypeDef lpf_alpha, lpf_beta;
extern PLL_HandleTypeDef pll;
extern LuenbergerObserver luenberger;
